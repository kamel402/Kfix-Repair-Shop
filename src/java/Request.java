    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (uemail != null) {
            ...
            try {
                ...
                String memorySize = request.getParameter("memory-size");
                String hardDeskSize = request.getParameter("harddesk-size");
                String problems = request.getParameter("problems");
                String repairCost = request.getParameter("repair-cost");
                String repairStatus = request.getParameter("repair-status");
                ...
                pStmt.setString(1, owner);                     // 1 specifies the first parameter in the query  
                pStmt.setString(2, serialNumber);             // 2 specifies the second parameter in the query
                pStmt.setString(3, type);
                pStmt.setString(4, model);
                pStmt.setString(5, memorySize);
                pStmt.setString(6, hardDeskSize);
                pStmt.setString(7, problems);
                if (repairCost == "") {
                    repairCost = null;
                }
                pStmt.setString(8, repairCost);
                if (repairStatus == "") {
                    repairStatus = null;
                }
                pStmt.setString(9, repairStatus);
                if (finishDate == "") {
                    finishDate = null;
                }
                pStmt.setString(10, finishDate);
                pStmt.setString(11, deliveryDate);
                ...
                if (k == 1) {
                    ...
                    out.println("<p>Your Request is Successfuly Registered</p>");
                } else // i.e.  k = 0
                {
